...
//				pm->submodel[n].triggers = NULL;
		...
		//key_getch();
			...
			case ID_TGUN:
			case ID_TMIS: {
				int n_banks = cfile::io::read<int>(fp);			// Number of turrets
				...
				for ( i = 0; i < n_banks; i++ ) {
					...
					parent = cfile::io::read<int>(fp);
					physical_parent = cfile::io::read<int>(fp);
...
int model_load(const char *filename, int n_subsystems, model_subsystem *subsystems, int ferror, int duplicate)
{
	...
	if(Fred_running && Parse_normal_problem_count > 0)
	{
		...
		SCP_Messagebox(MESSAGEBOX_ERROR, buffer, "Error");
	}
#endif
