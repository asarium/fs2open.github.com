...
void standalone_main_init()
{
   std_debug_set_standalone_state_string("Main Init");   
	...
	if((Multi_options_g.protocol == NET_IPX) && !Ipx_active){
		SCP_Messagebox(MESSAGEBOX_ERROR, XSTR( "You have selected IPX for multiplayer FreeSpace, but the IPX protocol was not detected on your machine.", 1402), "Error");
		exit(1);
	}
	if((Multi_options_g.protocol == NET_TCP) && !Tcp_active){
		if (Tcp_failure_code == WSAEADDRINUSE) {
			SCP_Messagebox(MESSAGEBOX_ERROR, XSTR("You have selected TCP/IP for multiplayer FreeSpace, but the TCP socket is already in use.  Check for another instance and/or use the \"-port <port_num>\" command line option to select an available port.", 1620), "Error");
		} else {
			SCP_Messagebox(MESSAGEBOX_ERROR, XSTR("You have selected TCP/IP for multiplayer FreeSpace, but the TCP/IP protocol was not detected on your machine.", 362), "Error");
		}
