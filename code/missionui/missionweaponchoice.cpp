	...
	if (new_ship)
	{
		...
		if(display_type < 0)
		{
			if ( wl_ship->overhead_bitmap < 0 )
			{
				...
				{
					// lo-res
					wl_ship->overhead_bitmap = bm_load_animation(sip->overhead_filename, NULL, NULL, 0, cfile::TYPE_INTERFACE);
				} else {
					// high-res
...
void wl_load_icons(int weapon_class)
{
	...
	{
		first_frame = bm_load_animation(Weapon_info[weapon_class].icon_filename, &num_frames, NULL, 0, cfile::TYPE_INTERFACE);
